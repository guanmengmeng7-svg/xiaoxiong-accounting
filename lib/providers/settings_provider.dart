...
class SettingsProvider extends ChangeNotifier {
  ...
  Future<void> setUserAvatarUrl(String? url) async {
    _userAvatarUrl = url;
    if (url != null) {
      await _prefs?.setString('user_avatar_url', url);
    } else {
  ...
  Future<void> setAiAvatarUrl(String? url) async {
    if (url != null && url.isNotEmpty) {
      _roleAvatars[_roleId] = url;
      await _prefs?.setString('avatar_$_roleId', url);
    } else {
      _roleAvatars.remove(_roleId);
      await _prefs?.remove('avatar_$_roleId');
    }
    notifyListeners();
  }
